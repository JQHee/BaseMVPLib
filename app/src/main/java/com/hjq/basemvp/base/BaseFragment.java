...
import com.hjq.basemvp.di.component.DaggerFragmentComponent;
import com.hjq.basemvp.di.component.FragmentComponent;
...
public abstract class BaseFragment<T extends BaseContract.BasePresenter> extends RxFragment implements BaseContract.BaseView {
    ...
    private void inflaterView(LayoutInflater inflater, @Nullable ViewGroup container) {
        if (mRootView == null) {
            if (getLayout() instanceof  Integer) {
                mRootView = inflater.inflate((Integer) getLayout(), container, false);
            } else if (getLayout() instanceof View) {
                mRootView =  (View) getLayout();
            } else {
                throw new ClassCastException("getLayout() type must be int or View");
